...
#Shiny Visualization code

P.S. : All the Shiny files are in the folder "Mashable_shares_prediction" in the [GitHub repo](https://github.com/abhijith-asok/BST-260-Final-project-Mashable-Prediction), which is also the name of the App. You could run it directly from the repo using the **runGitHub()** command. Alternatively, you could also view the dashboard on Shinyapps [here](https://abhijith-asok.shinyapps.io/mashable_shares_prediction/). Once again, in case you are not able to access the dashboard, kindly shoot an email to aasok@hsph.harvard.edu or chilger@hsph.harvard.edu and we'll restart the dashboard. RStudio's Shiny Server only allows a maximum of 1 hour of idle time before sending the dashboard to sleep.
